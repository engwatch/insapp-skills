...
| [report_claude_use](#report_claude_use) | `/report_claude_use` | Отчёт по использованию Claude Code: анализ сессий, группировка по активностям, HTML-дашборд | — |
...
**Команда:** `/report_claude_use [период]`

Анализирует все сессии Claude Code за указанный период (по умолчанию 3 дня), группирует по направлениям работы и генерирует красивый HTML-дашборд в светлых тонах.
...
3. Категоризирует каждую сессию по ключевым словам (МФО Отчёты, GitLab, Tracker, Договоры, Сверки и т.д.)
4. Формирует краткие итоги по каждой категории (не сырые логи, а человекочитаемые описания)
